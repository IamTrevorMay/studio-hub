          ...
          {platforms.map(p => {
            ...
            return (
              <div key={p} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                ...
                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.42)' }}>{meta.label}</span>
              </div>
            );
          })}
        </div>
      </div>
      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.42)', margin: '4px 0 12px' }}>
      ...
      <div style={{ overflowX: 'auto', position: 'relative' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxHeight: '360px' }}
          ...
          {yTicks.map(t => {
            ...
            return (
              <g key={t}>
                ...
                <text x={PAD.left - 8} y={y + 3} fill="rgba(255,255,255,0.42)" fontSize="10" textAnchor="end">
                  {formatCompact(t)}
                </text>
              </g>
            );
          ...
          {xTicks.map(t => {
            const x = xPos(t);
            return (
              <text key={t} x={x} y={H - 12} fill="rgba(255,255,255,0.42)" fontSize="10" textAnchor="middle">
                {t}d
              </text>
            );
          })}
          {/* Axis labels */}
          <text x={W / 2} y={H - 0} fill="rgba(255,255,255,0.32)" fontSize="10" textAnchor="middle">Days since publish</text>
          <text x={14} y={H / 2} fill="rgba(255,255,255,0.32)" fontSize="10" textAnchor="middle" transform={`rotate(-90, 14, ${H / 2})`}>Views (log scale)</text>
          {/* Dots */}
          {dots.map((d, i) => (
            <circle key={d.id}
              ...
              stroke={hoveredDot === i ? '#e7ebf2' : d.color} strokeWidth={hoveredDot === i ? 2 : 0.5}
        ...
        {hoveredDot !== null && dots[hoveredDot] && (() => {
          ...
          return (
            <div style={{
              ...
              <div style={{ fontSize: '12px', fontWeight: 600, color: '#e7ebf2', marginBottom: '4px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {d.title}
              </div>
              <div style={{ display: 'flex', gap: '8px', fontSize: '11px', color: 'rgba(255,255,255,0.55)' }}>
