      ...
      <div style={analysisStyles.cardHeader}>
        ...
        <div style={{ display: 'flex', gap: '16px', fontSize: '11px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <span style={{ color: 'rgba(255,255,255,0.42)' }}>Posts/week</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <span style={{ width: '12px', height: '2px', background: '#22c55e' }} />
            <span style={{ color: 'rgba(255,255,255,0.42)' }}>Followers gained</span>
          </div>
        </div>
      </div>
      <div style={{ overflowX: 'auto', position: 'relative' }}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxHeight: '320px' }}
          ...
          {[0, 0.25, 0.5, 0.75, 1].map(pct => {
            ...
            return <line key={pct} x1={PAD.left} y1={y} x2={W - PAD.right} y2={y} stroke="rgba(255,255,255,0.05)" />;
          })}
          {/* Zero line */}
          ...
          {linePoints.map((p, i) => (
            <circle key={i} cx={p.x} cy={p.y} r={hoveredWeek === i ? 4 : 2.5}
              fill="#22c55e" stroke="#161d2b" strokeWidth="1" />
          ))}
          {/* X-axis labels */}
          {data.map((d, i) => {
            ...
            const x = PAD.left + i * xStep;
            return (
              <text key={i} x={x} y={H - 8} fill="rgba(255,255,255,0.42)" fontSize="9" textAnchor="middle"
        ...
        {hoveredWeek !== null && data[hoveredWeek] && (() => {
          ...
          return (
            <div style={{
              ...
              <div style={{ fontSize: '11px', fontWeight: 600, color: '#e7ebf2', marginBottom: '6px' }}>Week of {d.week}</div>
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.55)', marginBottom: '3px' }}>
