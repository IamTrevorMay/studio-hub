...
export default function HarborHome({ onOpenRoom, onBackToLauncher }) {
  ...
  const fetchSessions = useCallback(async () => {
    const { data, error } = await supabase
      ...
      .select('id, title, status, mode, guest_token, scheduled_at, started_at, ended_at, archived_at, created_at')
  ...
  const createSession = async (e, sessionMode = 'recording') => {
    e?.preventDefault();
    if (creating) return;
    setCreating(true);
    // guest_token is generated by the DB column default. Meetings raise the
    // seat cap (mesh holds ~6) and default recording off (DB column default).
    const isMeeting = sessionMode === 'meeting';
    const { data, error } = await supabase
      .from('harbor_sessions')
      .insert({
        title: newTitle.trim() || (isMeeting ? 'Untitled meeting' : 'Untitled session'),
        created_by: profile?.id,
        mode: sessionMode,
        max_participants: isMeeting ? 6 : 4,
      })
      .select('id, title, status, mode, guest_token, scheduled_at, started_at, ended_at, archived_at, created_at')
      .single();
    if (error) {
      ...
      <div style={styles.inner}>
        ...
        <form onSubmit={createSession} style={styles.createRow}>
          <input
            ...
            placeholder="New session or meeting title…"
            maxLength={120}
          ...
          <button
            type="button"
            style={button({ variant: 'secondary', disabled: creating })}
            disabled={creating}
            onClick={(e) => createSession(e, 'meeting')}
            title="Start a Google-Meet-style call — no green room, recording off by default"
          >
            Start meeting
          </button>
        </form>
              ...
              <div key={s.id} style={styles.sessionCard}>
                <div style={styles.sessionInfo}>
                  <div style={styles.sessionTitleRow}>
                    ...
                    {s.mode === 'meeting' && <span style={pill('default')}>Meeting</span>}
