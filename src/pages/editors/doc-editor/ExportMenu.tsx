...
function nodeToMd(node: Record<string, unknown>, depth: number): string {
  ...
  switch (type) {
    ...
    case 'orderedList':
      return (content ?? []).map((child, i) => nodeToMd({ ...child, _orderedIndex: i + 1 }, depth)).join('') + '\n'
    case 'listItem': {
    ...
    case 'blockquote':
      return (content ?? []).map((child) => '> ' + nodeToMd(child, depth).trimEnd()).join('\n') + '\n\n'
    case 'codeBlock': {
  ...
  return content.map((node) => {
    ...
    for (const mark of marks) {
      ...
      switch (markType) {
        ...
        case 'link': result = `[${result}](${markAttrs.href as string})`; break
      }
    }
    return result
  }).join('')
...
<head>
  ...
  <style>
    body { font-family: Arial, sans-serif; font-size: 11pt; line-height: 1.15; color: #1f2937; max-width: 816px; margin: 40px auto; padding: 0 24px; }
    ...
    blockquote { border-left: 4px solid #d1d5db; padding-left: 16px; font-style: italic; color: #6b7280; margin: 16px 0; }
    code { background: #f3f4f6; color: #374151; padding: 2px 6px; border-radius: 4px; font-size: 13px; font-family: monospace; }
    pre { background: #111827; color: #e2e8f0; border-radius: 8px; padding: 16px; overflow-x: auto; margin: 16px 0; }
