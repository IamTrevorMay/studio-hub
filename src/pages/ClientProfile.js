...
export default function ClientProfile() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    setLoading(true);
    // The client_profiles row may not exist yet (created lazily), so maybeSingle.
    const [{ data: prof }, { data: clProf }] = await Promise.all([
      supabase.from('profiles').select('full_name, email, avatar_url').eq('id', profile.id).single(),
      supabase.from('client_profiles').select('*').eq('id', profile.id).maybeSingle(),
    ]);
    setForm({
      full_name: prof?.full_name || '',
      phone: clProf?.phone || '',
      company_name: clProf?.company_name || '',
      bio: clProf?.bio || '',
      drive_folder_url: clProf?.drive_folder_url || '',
    });
    setEmail(prof?.email || '');
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Upsert client_profiles because the row may not exist yet.
      ...
      <div style={styles.section}>
        ...
        <div style={styles.fieldGroup}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <label style={styles.fieldLabel}>Google Drive Folder URL</label>
            {form.drive_folder_url.trim() && (
              <a
                href={form.drive_folder_url.trim()}
                target="_blank"
                rel="noreferrer"
          ...
          <input
            ...
            onChange={e => updateField('drive_folder_url', e.target.value)}
