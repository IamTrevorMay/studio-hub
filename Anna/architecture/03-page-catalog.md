...
| `deliverables` | `Deliverables.js` | Deliverables | **167KB** | Sponsor deliverables / ad reads / "Brands" (UI alias for campaign — verified `brand`-named state throughout). Review-status pipeline, ad-copy, briefs (link/file/text), `AgencyThread` comment target. **Largest page in repo.** ⚠️ **Prop-name bug:** `AppLayout.js:860` passes `initialCampaignId`/`onCampaignOpened`, but the component destructures `initialBrandId`/`onBrandOpened` (`Deliverables.js:37`, used `:168-172`) — the deep-link auto-expand currently never fires. **Edit/close contract (mapped 2026-09-25):** a deliverable is closed by `handleSaveVideoLink` (`Deliverables.js:818`) → `sponsor_deliverables.update({ video_url, delivered: true, updated_at })` — no status enum change, no notification, no `revenue_events` write. The full edit form (`handleSaveDeliverable` `:681`) writes title/type/due_date(month+'-01')/ad_copy(only if dirty)/platforms/needs_review/campaign_id/pay/video_event_id/channel/review_status/video_url/`delivered = !!video_url`/review_due, then syncs the sponsor `calendar_events` row and calls `syncBrandRevenue(campaign_id)` (`:1027`, one `revenue_events` row per PAID campaign keyed `sponsor_campaign_<id>`). Neither path is `isAdmin`-gated (only Pay/Total Pay headers + slot limits are); RLS is `USING (true)` for authenticated (`20260720233000_remove_agency_portal.sql:19`). `DeliverablesMobile.js` mirrors both (Edit FullScreenSheet + Mark Delivered BottomSheet) since 2026-09-25. |
