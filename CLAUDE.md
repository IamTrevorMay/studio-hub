...
Editing is handed out from the Dashboard's `+ Assignment` menu (`AssignmentMenuButton` → `MemberAssignmentModal` or `ContractorAssignmentModal`). Both grew an optional **Slate item** field (shared `src/components/SlateItemPicker.js`); the client-mode contractor modal never shows it.
