...
// rc-stat-box.
// 2F.4 adds: rc-table, rc-bar-chart, rc-donut-chart, rc-statline.
// 2F.5 adds: rc-heatmap (16x16 spectrum + neighbor-interp + bilinear upscale
// + legend, plus legacy 5x5 count mode), rc-zone-plot, rc-movement-plot,
// the pitch-color palette, and the supporting spectrum + metric helpers.
//
...
function drawRCHeatmap(ctx, el, deps) {
  const p = el.props || {};
  ...
  // Plot bounds (feet): width 3.52, height 3.82 → aspect ~0.921.
  // Constrain plot rect to that aspect inside the available area so the
  // zone never looks stretched.
  ...
  if (providedGrid) {
    ...
    // 2 passes of neighbor interpolation for empty cells.
  ...
  // Per-pixel bilinear sample onto an off-screen canvas at plot resolution;
  // draw 1:1 with smoothing off so cell boundaries stay crisp.
  const targetW = Math.max(1, Math.round(plotW));
  const targetH = Math.max(1, Math.round(plotH));
  const off = deps.createCanvas(targetW, targetH);
...
async function renderSceneToPNG(scene, { createCanvas, GlobalFonts, loadImage }) {
  ...
  for (const el of sorted) {
    ...
    await drawElement(ctx, el, { loadImage, createCanvas });
