-- Phase 1: make daily_platform_rollups.avg_engagement_rate VIEW-WEIGHTED.
--
-- Previously avg_engagement_rate = avg(engagement_rate) across every post
-- published that day — a naive mean, so a single tiny post with an outlier
-- rate could dominate the platform's daily engagement number. We now weight by
-- each post's own views: sum(views * rate) / sum(views), which is the rate a
-- viewer actually experienced. engagement_rate is stored as a 0–1 fraction, so
-- the weighted result stays in 0–1 space (the frontend multiplies by 100).
--
-- Only the avg_engagement_rate expression changes; all other columns, the
-- unique index (required for CONCURRENTLY refresh), the date index, grants, and
-- the refresh function are identical to the 2026-06-29 baseline.
...
SELECT d.date,
   ...
   FROM generate_series(CURRENT_DATE - '730 days'::interval, CURRENT_DATE::timestamp without time zone, '1 day'::interval) d(date)
     ...
     LEFT JOIN LATERAL ( SELECT count(DISTINCT ci.id) AS posts_published,
            ...
            -- View-weighted engagement: a post with more views counts more.
            -- Falls back to a simple mean only when no post has views.
            COALESCE(
              sum(cm_latest.views * cm_latest.engagement_rate)
                / NULLIF(sum(cm_latest.views), 0),
              avg(cm_latest.engagement_rate)
