// Move a card to a target column (admin drag-drop).
// Skips any open tasks in the vacated column, sets current_step_key,
// then enters target column (creating new tasks or blocking if empty).
...
Deno.serve(async (req: Request) => {
  ...
  // Skip vacated column's open tasks.
  let skipped: string[] = [];
  ...
  // Reopen instance if it was complete (rare but possible via backward drag).
