...
serve(async (req) => {
  try {
    // Auth: CRON_SECRET (header or query) or admin JWT required. This handler
    // runs under service role and performs destructive resets + YouTube API
    // calls, so it must not be reachable by unauthenticated traffic.
    {
