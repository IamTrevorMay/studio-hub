...
export default function FindReplace({ editor, onClose }: Props) {
  ...
  useEffect(() => {
    return () => {
      ...
      if (storage) {
        storage.searchTerm = ''
        // Force a transaction to refresh decorations
  ...
  // Sync search state into the plugin storage and force decoration refresh
  useEffect(() => {
    const storage = editor.storage.searchHighlight
    if (!storage) return
    ...
    // Dispatch a no-op transaction to trigger the plugin's apply()
    editor.view.dispatch(editor.state.tr)
